...
params_2 = [
    {'name': 'Global parameters','expanded': False, 'type': 'group', 'children': [
		...
		{'name': 'Shaping (0.25, 2, 0.5, 2 usec)', 'type': 'list', 'values': [0,1,2,3], 'value': 2, 'tip':"Shaping (0.25, 2, 0.5, 2 usec) (T0:T1)"},
        {'name': 'Threshold (PA0:PA9)', 'type': 'int', 'limits': (0, 1023), 'value': 200, 'tip':"Threshold"},
                ]},
...
def save():
    ...
    t0 = int(p2.param('Global parameters','Shaping (0.25, 2, 0.5, 2 usec)').value()) << 11
...
def load_MARS():
    ...
    if (mars_addr == 0):
	    ...
	    print "choose valid asic address!"
